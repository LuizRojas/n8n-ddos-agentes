...
class FeaturesInput(BaseModel):
    ...
    request_count: int = Field(..., description="The total number of requests in the log window")
    error_rate: float = Field(..., description="The ratio of error status codes (4xx/5xx) to total requests")
    average_request_frequency: float = Field(..., description="The average time in seconds between requests")
    average_bytes_sent: float = Field(..., description="The average number of bytes sent in a response")
    

    client_real_ip: str = Field("UNKNOWN", description="Real client IP, injected for context.")


class PredictionOutut(BaseModel):
    """
